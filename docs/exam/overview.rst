...
Example header for the solution (one Python file, file extension: .py):
...
Exam 1: Nahuelbuta weather station
-----------------------------------
...
Exam 2: A GPS Station in Antarctica
------------------------------------
...
Visualize the vertical displacement as a function of time. The data is a bit noisy for your liking. Smooth
...
Exam 7: A bit of geophysics, but not too much.
-----------------------------------------------
...
* Read in the CSV file ``data8.csv``. (use ``np.loadtxt()``, ``delimiter=","`` and ``usecols=(1,2)``)
* Write a function that filters out the invalid values (``-9999.0``) from the data.
